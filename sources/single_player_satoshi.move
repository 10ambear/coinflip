module coinflip::single_player_satoshi {
    // Imports
    ...
    // Counter library
    use coinflip::counter_nft::Counter;

    // HouseData library
    use coinflip::house_data::HouseData;
    ...
    public fun start_game(guess: String, counter: &mut Counter, coin: Coin<SUI>, house_data: &mut HouseData, ctx: &mut TxContext): ID {
        ...
        let (id, new_game) = internal_start_game(guess, counter, coin, house_data, fee_bp, ctx);  // this returns (ID, Game), the name is not great

        dof::add(house_data.borrow_mut(), id, new_game);
        // dof::add(...): This is a method call to dynamically add a field to an object. In the Move language, dynamic fields allow for flexible storage structures where the keys are not known ahead of time.
        // house_data.borrow_mut(): This borrows a mutable reference to the house_data object. The borrow_mut() function is used to get a mutable handle to the object so that changes can be made to its contents.
        // id: This is the identifier under which the new_game object will be stored within the house_data object. Essentially, it acts as a key for accessing the new_game object later.
        // new_game: This is the object being added to the house_data object under the specified id.
        id
    }
    ...
    fun internal_start_game(guess: String, counter: &mut Counter, coin: Coin<SUI>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
        ...
        map_guess(guess); // returns a u8 from a string (heads=0, tail=1)
